...
<g class='axis x-axis' class:snapTicks>
    {#each tickVals as tick, i}
        <g class='tick tick-{ i }' transform='translate({$xScale(tick)},{$yRange[0]})'>
            ...
            {/if}
            {#if tickMarks === true}
                <line class="tick-mark" y1='{0}' y2='{6}' x1='{xTick || isBandwidth ? $xScale.bandwidth() / 2 : 0}'
            ...
            <text
                    ...
                    cursor="pointer"
                    font-weight={highlightGroup === tick ? "bold":"normal"}
                    on:click={handleClick(tick)}>{formatTick(tick)}</text>
        </g>
    {/each}
    {#if baseline === true}
